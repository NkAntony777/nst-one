...
if __name__ == "__main__":
    ...
    if uploaded_file is not None:
        content_image = load_image(
            uploaded_file, (img_width, img_height), col=col1)
    ...
    if uploaded_file_style is not None:
        style_image = load_image(
            uploaded_file_style, (img_width_style, img_height_style), col=col2)
        ...
        col4.image(np.array(stylized_image[0]))

        # Download button for stylized image
        st.markdown("## Download Stylized Image")
        if st.button("Download Stylized Image"):
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as temp_file:
                ...
                st.markdown(f"Download the image [here]({temp_file.name})")
